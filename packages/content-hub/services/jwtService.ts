...
export class JWTService {
  ...
  decodeToken(token: string): MeiliSearchJWTClaims | null {
    try {
      ...
      if (parts.length !== 3) {
        throw ErrorHandler.createError('Invalid JWT format', 'INVALID_JWT');
      }
      ...
      if (this.config.enableDebugLogging) {
        Logger.debug('Decoded JWT claims:', decoded);
      }
  ...
  createTestToken(userId: string, email: string, options: {
    ...
    if (this.config.IS_PRODUCTION) {
      throw ErrorHandler.createError('Test tokens cannot be created in production', 'PRODUCTION_ERROR');
    }
    ...
    if (this.config.enableDebugLogging) {
      Logger.warn('Created test JWT token (development only):', {
        userId,
        email,
        claims: payload,
        token: testToken
