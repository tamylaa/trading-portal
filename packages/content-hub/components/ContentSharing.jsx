...
import { ErrorHandler } from '@tamyla/shared/utils';
import '../styles/ContentSharing.css';
  ...
  const makeAllFilesPublic = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw errorHandler.createError('Failed to make files public', 'FILE_PUBLIC_ERROR');
      }
    } catch (error) {
      Logger.error('Error making files public:', error);
      onError?.(error);
    } finally {
      setLoading(false);
    }
  ...
  const sendEmails = async () => {
    ...
    try {
      ...
      if (successful > 0) {
        ...
        throw errorHandler.createError('All emails failed to send', 'EMAIL_SEND_ERROR');
      }
