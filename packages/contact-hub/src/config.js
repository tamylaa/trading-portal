...
const CONTACT_HUB_DEFAULTS = {
  ...
  behavior: {
    defaultView: process.env.REACT_APP_CONTACT_DEFAULT_VIEW || 'list', // 'list' | 'grid' | 'table'
    itemsPerPage: parseInt(process.env.REACT_APP_CONTACT_ITEMS_PER_PAGE) || 25,
    maxFileSize: parseInt(process.env.REACT_APP_CONTACT_MAX_FILE_SIZE) || 10 * 1024 * 1024, // 10MB for imports
  ...
  performance: {
    ...
    cacheTTL: parseInt(process.env.REACT_APP_CONTACT_CACHE_TTL) || 300000, // 5 minutes
...
export const configManager = createHubConfigManager('contact-hub', CONTACT_HUB_DEFAULTS);
