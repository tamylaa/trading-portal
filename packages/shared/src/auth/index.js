...
export class AuthService {
  ...
  scheduleRefresh() {
    ...
    this.refreshTimeout = setTimeout(async () => {
      try {
        ...
        this.notifyListeners('authError', { error: 'Token refresh failed' });
      }
    }, threshold);
  }
  ...
  async login(credentials) {
    try {
      ...
      this.notifyListeners('authError', { error: error.message });
  ...
  async refreshAccessToken() {
    ...
    try {
      ...
      this.notifyListeners('tokenRefreshed', { user: this.user });
      return { success: true };
    } catch (error) {
